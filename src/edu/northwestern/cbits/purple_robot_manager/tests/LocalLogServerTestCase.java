...
public class LocalLogServerTestCase extends RobotTestCase
{
    ...
    public void test()
    {
        ...
        try
        {
            ...
            String oldEndpoint = LogManager.getInstance(this._context).getEndpoint();
            boolean oldEnabled = LogManager.getInstance(this._context).getEnabled();

            LogManager.getInstance(this._context).setEndpoint("http://127.0.0.1:12345/log");

            Assert.assertEquals("LLST001", "http://127.0.0.1:12345/log", LogManager.getInstance(this._context)
                    .getEndpoint());
            ...
            prefs = PreferenceManager.getDefaultSharedPreferences(this._context);

            Assert.assertTrue("LLST003", prefs.getInt(LogServerEmulatorRequestHandler.LOG_COUNT, 0) > 0);

            LogManager.getInstance(this._context).setEndpoint(oldEndpoint);

            Assert.assertEquals("LLST004", "" + oldEndpoint, "" + LogManager.getInstance(this._context).getEndpoint());

            LogManager.getInstance(this._context).setEnabled(oldEnabled);

            Assert.assertEquals("LLST005", oldEnabled, LogManager.getInstance(this._context).getEnabled());
        }
