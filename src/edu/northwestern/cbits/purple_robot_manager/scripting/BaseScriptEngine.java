...
public abstract class BaseScriptEngine
{
    ...
    private static final String DEFAULT_SMS_NUMBER = "config_default_sms_number";
    ...
    @ScriptingEngineMethod(language = "All", assetPath = "all_schedule_script_delayed.html", category = R.string.docs_script_category_dialogs_notifications, arguments = { "identifier", "delay", "script" })
    public void scheduleScriptDelayed(String identifier, long delay, String script)
    {
        ScheduleManager.updateScript(this._context, identifier, delay, script);
    }
    ...
    public List<String> fetchToneList()
    {
        ...
        Collections.addAll(tones, included);
    ...
    @ScriptingEngineMethod(language = "All", assetPath = "all_set_default_sms_number.html", category = R.string.docs_script_category_todo, arguments = { "phoneNumber" })
    public void setDefaultSmsNumber(String phoneNumber)
    {
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(this._context);
        ...
        if (phoneNumber != null && phoneNumber.trim().length() > 0) {
            e.putString(BaseScriptEngine.DEFAULT_SMS_NUMBER, phoneNumber.trim());
        } else {
            e.remove(BaseScriptEngine.DEFAULT_SMS_NUMBER);
        }

        e.apply();
    }

    @ScriptingEngineMethod(language = "All", assetPath = "all_get_default_sms_number.html", category = R.string.docs_script_category_todo, arguments = { })
    public String getDefaultSmsNumber()
    {
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(this._context);

        return prefs.getString(BaseScriptEngine.DEFAULT_SMS_NUMBER, null);
    }

    @ScriptingEngineMethod(language = "All", assetPath = "all_send_sms_message.html", category = R.string.docs_script_category_todo, arguments = { "message", "destination"})
    public void sendSmsMessage(String message, String destination) {
        if (ContextCompat.checkSelfPermission(this._context, Manifest.permission.SEND_SMS) == PackageManager.PERMISSION_GRANTED) {
            if (destination == null) {
                destination = this.getDefaultSmsNumber();
            }
    ...
    @ScriptingEngineMethod(language = "All", assetPath = "all_send_sms_message.html", category = R.string.docs_script_category_todo, arguments = { "message" })
    public void sendSmsMessage(String message) {
        this.sendSmsMessage(message, null);
    }
}
