...
public class AmbientHumidityProbe extends ContinuousProbe implements SensorEventListener
{
    ...
    private long buffersIndex = 0;

    private final float valueBuffer[][][] = new float[NUM_BUFFERS][1][BUFFER_SIZE];
    private final double timeBuffer[][] = new double[NUM_BUFFERS][BUFFER_SIZE];
    private final double sensorTimeBuffer[][] = new double[NUM_BUFFERS][BUFFER_SIZE];
    ...
    public void onSensorChanged(SensorEvent event)
    {
        ...
        if (this.passesThreshold(event))
        {
            synchronized (this)
            {
                int which = (int) (buffersIndex % NUM_BUFFERS);

                sensorTimeBuffer[which][bufferIndex] = event.timestamp;
                timeBuffer[which][bufferIndex] = now / 1000;

                valueBuffer[which][0][bufferIndex] = event.values[0];

                bufferIndex += 1;

                if (bufferIndex >= timeBuffer[which].length)
                {
                    buffersIndex += 1;
                    ...
                    data.putDoubleArray(ContinuousProbe.EVENT_TIMESTAMP, timeBuffer[which]);
                    data.putDoubleArray(ContinuousProbe.SENSOR_TIMESTAMP, sensorTimeBuffer[which]);

                    for (int i = 0; i < fieldNames.length; i++)
                    {
                        data.putFloatArray(fieldNames[i], valueBuffer[which][i]);
                    }
