                ...
                if (bufferIndex >= this._currentTimeBuffer.length)
                {
                    ...
                    Runnable r = new Runnable() {
                        ...
                        public void run() {
                            ...
                            double[] normalBuffer = new double[mySensorTimeBuffer.length];

                            double start = mySensorTimeBuffer[0];
                            double end = mySensorTimeBuffer[mySensorTimeBuffer.length - 1];
                            double tick = (end - start) / mySensorTimeBuffer.length;

                            start = myTimeBuffer[0] * (1000 * 1000);
                            ...
                            if (myTimeBuffer.length > 0)
                            {
                                ...
                                if (!Double.isNaN(x) && !Double.isNaN(y) && !Double.isNaN(z))
                                {
                                    ...
                                    ProbeValuesProvider.getProvider(me._context).insertValue(me._context, AccelerometerProbe.DB_TABLE, me.databaseSchema(), values);
                                }
                            }
