        ...
        if (this._pollIntent == null)
        {
            ...
            context.registerReceiver(new BroadcastReceiver()
            {
                ...
                public void onReceive(final Context context, Intent intent)
                {
                    ...
                    PowerManager.WakeLock lock = WakeLockManager.getInstance(context).requestWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "app-launch-probe");
                    ...
                    else
                    {
                                        ...
                                        } else
                                        {
                                            ...
                                            while (events.getNextEvent(event)) {
                                                if (event.getEventType() == UsageEvents.Event.MOVE_TO_FOREGROUND) {
                                                    ...
                                                    me._lastPkgName = event.getPackageName();
                        ...
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                                alarm.setExactAndAllowWhileIdle(AlarmManager.RTC_WAKEUP, System.currentTimeMillis() + 60000, me._pollIntent);
                            } else {
                                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                                    alarm.setExact(AlarmManager.RTC_WAKEUP, System.currentTimeMillis() + 60000, me._pollIntent);
                                }
                            }
                        }
                    }
        ...
        synchronized (this)
        {
            if (set && me._lastCheck == -1)
            {
                ...
                if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
                    ...
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                        alarm.setExactAndAllowWhileIdle(AlarmManager.RTC_WAKEUP, System.currentTimeMillis(), this._pollIntent);
                    } else {
                        alarm.setExact(AlarmManager.RTC_WAKEUP, System.currentTimeMillis(), this._pollIntent);
                    }
                }
            }
        }
