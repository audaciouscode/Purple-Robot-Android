                ...
                synchronized (me)
                {
                    try
                    {
                        ...
                        if (filenames == null || found.intValue() == 0)
                        {
                            if (me._regularFilenames.size() == 0) {
                                ...
                                if (regularFilenames != null) {
                                    Collections.addAll(me._regularFilenames, regularFilenames);
                                }
                            }
                        ...
                        if (result == DataUploadPlugin.RESULT_SUCCESS)
                        {
                            ...
                            {
                                File archive = new File(me.getArchiveFolder(), filename + ".archive");

                                payloadFile.renameTo(archive);
                            }
                            else
                                payloadFile.delete();
    ...
    public void processIntent(final Intent intent)
    {
        ...
        synchronized (this)
        {
            ...
            else if (Probe.PROBE_TRANSMIT_BUFFER.equals(action))
            {
                ...
                properties.put("number_converted", pendingFolder.list(filter).length);

                LogManager.getInstance(context).log("pr_converted_on_demand_payloads", properties);
