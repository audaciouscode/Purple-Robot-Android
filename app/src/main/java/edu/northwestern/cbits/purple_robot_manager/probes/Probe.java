...
public abstract class Probe
{
    ...
    public static final String PROBE_TRANSMIT_MODE = "transmit-mode";
    ...
    public void updateFromMap(Context context, Map<String, Object> params)
    {
        if (params.containsKey(Probe.PROBE_ENABLED))
        {
            ...
            if (transmitMode != null) {
                if ("normal".equals(transmitMode.toString().toLowerCase())) {
                    this.setTransmitMode(context, Probe.PROBE_TRANSMIT_MODE_NORMAL);
                } else if ("priority".equals(transmitMode.toString().toLowerCase())) {
                    this.setTransmitMode(context, Probe.PROBE_TRANSMIT_MODE_PRIORITY);
                } else if ("on-demand".equals(transmitMode.toString().toLowerCase())) {
                    this.setTransmitMode(context, Probe.PROBE_TRANSMIT_MODE_ON_DEMAND);
                }
            }
        }
    }
    ...
    public JSONObject fetchSettings(Context context)
    {
        ...
        try
        {
            ...
            JSONObject transmitMode = new JSONObject();
            transmitMode.put(Probe.PROBE_TYPE, Probe.PROBE_TYPE_STRING);
            values = new JSONArray();
            values.put("normal");
            values.put("priority");
            values.put("on-demand");
            enabled.put(Probe.PROBE_VALUES, values);
            settings.put(Probe.PROBE_ENABLED, transmitMode);
        }
