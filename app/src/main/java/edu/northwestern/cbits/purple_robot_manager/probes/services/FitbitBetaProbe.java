...
//    private static final String LAST_ELEVATION_TIMESTAMP = "config_fitbit-beta_last_elevation_timestamp";
    ...
    {
        Keystore.put(FitbitApi.OAUTH2_CLIENT_ID, context.getString(R.string.fitbit_client_id));
        ...
        Keystore.put(FitbitApi.CALLBACK_URL, "http://purple.robot.com/oauth/fitbit-beta");
    }
        ...
        if (super.isEnabled(context))
        {
            if (prefs.getBoolean(FitbitBetaProbe.ENABLED, false))
            {
                ...
                else
                {
                    ...
                    Keystore.put(FitbitApi.USER_ACCESS_TOKEN, prefs.getString(FitbitBetaProbe.OAUTH_ACCESS_TOKEN, null));
                    Keystore.put(FitbitApi.USER_REFRESH_TOKEN, prefs.getString(FitbitBetaProbe.OAUTH_REFRESH_TOKEN, null));
                    Keystore.put(FitbitApi.USER_TOKEN_EXPIRES, "" + expires);
                            ...
                            public void run()
                            {
                                try
                                {
                                    if (System.currentTimeMillis() > expires) {
                                        FitbitApi.refreshTokens(context, FitbitBetaProbe.OAUTH_ACCESS_TOKEN, FitbitBetaProbe.OAUTH_REFRESH_TOKEN, FitbitBetaProbe.OAUTH_TOKEN_EXPIRES);
                                    ...
                                    for (long minute : minutesToEval)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if (prefs.getBoolean(FitbitBetaProbe.ENABLE_STEPS, FitbitBetaProbe.DEFAULT_ENABLE_STEPS)) {
                                                try {
                                                    JSONObject stepsObj = FitbitApi.fetch(Uri.parse("https://api.fitbit.com/1/user/-/activities/steps/date/" + dayString + "/1d/1min.json"));
                                            ...
                                            if (prefs.getBoolean(FitbitBetaProbe.ENABLE_CALORIES, FitbitBetaProbe.DEFAULT_ENABLE_CALORIES)) {
                                                ...
                                                try {
                                                    JSONObject stepsObj = FitbitApi.fetch(Uri.parse("https://api.fitbit.com/1/user/-/activities/calories/date/" + dayString + "/1d/1min.json"));
                                            ...
                                            if (prefs.getBoolean(FitbitBetaProbe.ENABLE_DISTANCE, FitbitBetaProbe.DEFAULT_ENABLE_DISTANCE)) {
                                                try {
                                                    JSONObject stepsObj = FitbitApi.fetch(Uri.parse("https://api.fitbit.com/1/user/-/activities/distance/date/" + dayString + "/1d/1min.json"));
                                            ...
                                            if (prefs.getBoolean(FitbitBetaProbe.ENABLE_FLOORS, FitbitBetaProbe.DEFAULT_ENABLE_FLOORS)) {
                                                try {
                                                    JSONObject stepsObj = FitbitApi.fetch(Uri.parse("https://api.fitbit.com/1/user/-/activities/floors/date/" + dayString + "/1d/1min.json"));
                                            ...
                                            if (prefs.getBoolean(FitbitBetaProbe.ENABLE_HEART, FitbitBetaProbe.DEFAULT_ENABLE_HEART)) {
                                                try {
                                                    JSONObject stepsObj = FitbitApi.fetch(Uri.parse("https://api.fitbit.com/1/user/-/activities/heart/date/" + dayString + "/1d/1min.json"));
                                            ...
                                            if (prefs.getBoolean(FitbitBetaProbe.ENABLE_ELEVATION, FitbitBetaProbe.DEFAULT_ENABLE_ELEVATION)) {
                                                try {
                                                    JSONObject stepsObj = FitbitApi.fetch(Uri.parse("https://api.fitbit.com/1/user/-/activities/elevation/date/" + dayString + "/1d/1min.json"));
