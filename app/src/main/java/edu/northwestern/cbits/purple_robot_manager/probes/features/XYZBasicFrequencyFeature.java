...
public abstract class XYZBasicFrequencyFeature extends ContinuousProbeFeature
{
    ...
    private double[] _timestamps = null;
    ...
    private void appendValues(float[] incomingX, float[] incomingY, float[] incomingZ, double[] ts)
    {
        if (this._xValues == null) {
            this._xValues = new double[BUFFER_SIZE];
        }

        if (this._yValues == null) {
            this._yValues = new double[BUFFER_SIZE];
        }

        if (this._zValues == null) {
            this._zValues = new double[BUFFER_SIZE];
        }

        if (this._timestamps == null) {
            this._timestamps = new double[BUFFER_SIZE];
        }
